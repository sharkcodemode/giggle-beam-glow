(() => {
  if (globalThis.__ACTO_STORE_BRIDGE__) return;
  globalThis.__ACTO_STORE_BRIDGE__ = true;

  const MODAL_ID = "acto-store-bridge-modal";
  const STYLE_ID = "acto-store-bridge-style";
  const DOCK_BUTTON_ATTR = "data-acto-store-dock-button";
  ...
  function storeIcon() {
    return iconSvg("M6 8V6a6 6 0 0 1 12 0v2M4 8h16l-1 13H5L4 8Z M9 12h6", "w-4 h-4");
  }
  ...
  function renderStoreBody() {
    ...
    const panel = modal.querySelector(".acto-store-window");
    if (!panel) return;
    panel.innerHTML = `${renderHeader()}${selectedModuleId ? renderModule() : renderHome()}`;
    setWindowPosition();
    wireDrag();
    wireStoreEvents();
  }
  ...
  function openStore() {
    ...
    runtime.sendMessage({ type: OPEN_STORE_MODAL_MESSAGE_TYPE }, (response) => {
      const error = runtime.lastError;
      if (error || !response?.ok || !response?.opened) {
        console.warn("Nao foi possivel abrir a Loja fora do painel.", error || response?.error || response);
      }
    });
  }

  function createStoreDockButton(reference) {
    const button = reference.cloneNode(true);
    button.setAttribute(DOCK_BUTTON_ATTR, "1");
    button.setAttribute("title", "Loja");
    button.setAttribute("aria-label", "Loja");
    button.removeAttribute("data-state");

    const firstSvg = button.querySelector("svg");
    if (firstSvg) firstSvg.outerHTML = storeIcon();
    const label = Array.from(button.querySelectorAll("span")).find((span) => normalizeText(span.textContent) === "Music");
    if (label) label.textContent = "Loja";
  ...
  function ensureDockButton() {
    ...
    const reference =
      ...
      Array.from(document.querySelectorAll("button")).find((button) => normalizeText(button.textContent) === "Music");
    const parent = reference?.parentElement;
    if (!reference || !parent) return;

    const button = createStoreDockButton(reference);
  ...
  document.addEventListener(
    "click",
    (event) => {
      const button = event.target?.closest?.(`[${DOCK_BUTTON_ATTR}], button[title="Loja"], button[aria-label="Loja"]`);
  ...
  globalThis.actoOpenStore = openStore;
})();
