...
const ACTO_NATIVE_MASK_TITLE = "Acto: Msg Recebida\n\né na extensao ?tem que fazer alguma modificação ?";
...
async function actionLovableProxy(captured: Captured, params: Record<string, unknown>) {
  ...
  if (method === "POST" && /^\/projects\/[^/]+\/chat\/?$/.test(u.pathname)) {
    const source = requestBody && typeof requestBody === "object"
      ...
      : {};
    requestBody = buildThinkingPayload(
...
function getSelectedElements(context: Record<string, unknown>): unknown[] {
  if (Array.isArray(context.selectedElements)) return context.selectedElements;
  if (Array.isArray(context.selected_elements)) return context.selected_elements;
  return [];
}
...
function getElementText(element: unknown): string {
  ...
  if (isStr(record.textContent)) return record.textContent.trim();
  const textNodes = Array.isArray(record.textNodes) ? record.textNodes : [];
  return textNodes.map((node) => {
    if (!node || typeof node !== "object") return "";
    const item = node as Record<string, unknown>;
    return isStr(item.content) ? item.content : isStr(item.textContent) ? item.textContent : "";
...
async function actionSendMessage(captured: Captured, params: Record<string, unknown>, licenseId = "") {
  ...
  // `model: null` preserva exatamente o formato do payload Thinking capturado.
  ...
  console.log("[acto-v2 send] route=direct_thinking_visual_edit status=", res.status,
...
async function handleLegacy(
  ...
  if (hasSession) {
    ...
    console.log("[acto-v2 legacy]", rid,
      "route=direct_thinking_visual_edit",
      "license_source=", licenseSource,
      "license_ms=", sessionCheckMs,
      "used_gateway=false",
      "intent=visual_edit",
      "system_included=true",
  ...
  const bgTask = (async () => {
    ...
    try {
      ...
      console.log(
        "[acto-v2 legacy bg] route=direct_thinking_visual_edit project_id=", projectId,
  ...
  console.log(
    ...
    "route=direct_thinking_visual_edit project_id=", projectId,
    "license_source=", licenseSource,
    "used_gateway=false",
    "intent=visual_edit",
    "system_included=true",
...
// Qualquer payload Lovable montado aqui usa Thinking/Visual Edit.
// passthrough SSE direto. Toda lógica "inteligente" vive aqui no servidor.
// ───────────────────────────────────────────────────────────────────────
async function handleThinkingRelay(req: Request): Promise<Response> {
  let body: Record<string, unknown>;
  try {
    body = await req.json();
  } catch {
    return new Response(JSON.stringify({ ok: false, error: "thinking_relay_json_invalido" }), {
      status: 400,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
  ...
  const relayContext = body.context && typeof body.context === "object"
    ? { ...body, ...body.context as Record<string, unknown> }
    : body;
  const lovablePayload = buildThinkingPayload(
    msgId,
    finalMessage,
  ...
  for (let index = 0; index < tokenCandidates.length; index++) {
    ...
    console.log(
      "[acto-v2 token attempt] route=thinking_visual_edit_relay index=", index,
      "token=", describeChatToken(token, projectId),
  ...
  respHeaders.set("x-acto-relay", "thinking_visual_edit");
...
async function handle(req: Request): Promise<Response> {
  ...
  // Entrada legada é normalizada para o payload Thinking/Visual Edit atual.
  if (req.headers.get("x-acto-action") === "fix_relay") {
    return await handleThinkingRelay(req);
  }
  ...
  if (req.headers.get("x-acto-action") === "gateway_stream") {
    ...
    return jsonErr(
      "gateway_disabled",
      "Gateway desativado. O envio agora usa Thinking/Visual Edit direto.",
      200,
    );
  }
  ...
  try {
    ...
    switch (pt.action) {
      ...
      case "sheets_append":
        ...
        break;
      case "gateway_chat": {
        // BLOQUEADO: o envio agora usa Thinking/Visual Edit direto. Mantemos o código de
        ...
        data = {
          ...
          message: "Gateway desativado. O envio agora usa Thinking/Visual Edit direto.",
        };
        break;
      }
      default:
